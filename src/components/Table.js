            ...
            {!isLoading ? (
              <>
                {data.map((entry) => (
                  ...
                  >
                    <td className="flex flex-col justify-center items-center px-1 py-1 whitespace-nowrap">
                      <>{entry.name}</>
                      {entry.orgname ? (
                    ...
                    {entry.timeIn ? (
                      <td className="px-1 py-1 whitespace-nowrap">
                        <button
                          onClick={() =>
                            handleResetTimeIn(entry.id, entry.name)
                          }
                          type="button"
                    ...
                    {show.timeout &&
                      (entry.timeOut ? (
                        <td className="px-1 py-1 whitespace-nowrap">
                          <button
                            onClick={() =>
                              handleResetTimeOut(entry.id, entry.name)
                            }
                            type="button"
                      ...
                      <td className="px-1 py-1 whitespace-nowrap">
                        {show.claimed &&
                          (entry.claimed == 1 ? (
                            <button
                              onClick={() => {
                                handleUnclaim(entry.id, entry.name);
                              }}
                              type="button"
                            ...
                            <button
                              onClick={() => {
                                handleClaim(entry.id, entry.name);
                              }}
                              type="button"
        ...
        {isLoading && (
          <div className="flex flex-col items-center h-96 bg-white p-5">
            ...
            <p className="mt-2 text-sm">
              Currently Processing. If this notification persists for 5 seconds.
            </p>
            <p className=" text-sm">
              Possible Problems: Failed to Connect to Server or Internet is Slow
            </p>
            <p className="mt-2 text-sm italic">
              Possible Solution: Refresh your Browser or Restart your Device.
            </p>
          </div>
        )}
      </div>
